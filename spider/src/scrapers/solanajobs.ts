// ============================================================
// RemoteWeb3 Spider - Solana Jobs Scraper
// ============================================================
...
export class SolanaJobsScraper extends BaseScraper {
  constructor() {
    super({
      name: 'jobs.solana.com',
      baseUrl: 'https://jobs.solana.com',
      jobListUrl: 'https://jobs.solana.com/jobs',
    });
  }
  ...
  async scrape(): Promise<ScrapeResult> {
    ...
    try {
      ...
      for (let page = 1; page <= MAX_PAGES; page++) {
        ...
        const cards = $(
          'a[href*="/job/"], a[href*="/jobs/"], .job-card, .job-listing, .job-item, article.job, [class*="job-card"], [class*="job-listing"], .position-card, .opportunity-item, [class*="opening"]'
        ).toArray();
        ...
        for (const el of cards) {
          ...
          try {
            ...
            const titleEl = isAnchor
              ? card.find('h2, h3, h4, .title, .job-title, [class*="title"], [class*="name"]').first()
              : card.find('h2, h3, h4, .title, .job-title, [class*="title"], [class*="name"]').first();
            ...
            const existing = await checkJobExists(sourceId, 'jobs.solana.com');
            if (existing) {
            ...
            const departmentEl = card.find(
              '.department, .team, .category, [class*="department"], [class*="team"], [class*="category"]'
            ).first();
            const department = departmentEl.text().trim() || '';

            const locationEl = card.find(
              '.location, .job-location, [class*="location"], .remote-tag, .workplace'
            ).first();
            const locationText = locationEl.text().trim() || 'Remote';

            const typeEl = card.find(
              '.type, .job-type, .employment-type, [class*="type"], .commitment'
            ).first();
            const typeText = typeEl.text().trim() || '';

            const tagEls = card.find(
              '.tag, .badge, .skill, [class*="tag"], [class*="skill"]'
            );
            const tags: string[] = [];
            tagEls.each((_, t) => {
            ...
            const description = descEl.text().trim() || `${title} at Solana`;
            const companyName = department || 'Solana';
